        ...
        <section className='container max-w-6xl mx-auto flex flex-col gap-12 items-center py-36 px-6 relative min-h-screen '>
            <header className="flex justify-between gap-8">
                <div className="flex flex-col  flex-1">
                    <Badge variant={'outline'} className='mb-2 w-fit bg-gradient-to-b from-transparent to-zinc-800/80 rounded-full '>Quality Approved</Badge>
                    <h1 className='mb-8 self-start text-5xl [&>*]:font-display [text-shadow:0_2px_16px_rgba(174,207,242,.44)]'> <span className='bg-clip-text text-transparent bg-gradient-to-b from-zinc-400 from-10% to-55% to-white' >Trusted by experts</span></h1>
                    ...
                    <dl className="flex flex-col gap-8 md:flex-row justify-between">
                        {stats.map((stat, i) => (
                            <div key={stat.id} className={classNames(" flex relative md:px-10 flex-col gap-y-2", {
                                ...
                                <dt className="[text-shadow:0_2px_16px_rgba(174,207,242,.44)]  text-4xl font-display tracking-tight  sm:text-5xl">
                                    {stat.value}
                                </dt>
